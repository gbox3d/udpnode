...
function save_BootStatus() file.open("status.json", 'w') file.write(sjson.encode(boot_status)) file.close() end
------------------------------------
...
function system_start_up()
    ...
    if file.exists("config.json") then
        ...
        dofile("config.lua")
        file.open("config.json", 'w') file.write(sjson.encode(app_config)) file.close()
    end
...
function startup()
    ...
    packet_dic = {
        default = function() local rt = {result = 'nocmd'} udp_server:send(sjson.encode(rt))  end
    }
    ...
    function processRecv(s,c,_port,_ip)
        ...
        if c:byte(1,1) == 123 then -- check '{}'
            ...
            if packet.cmd ~= nil then
                ...
                if extraRecvCb ~= nil then extraRecvCb(packet) end
            end
        ...
        timerid_Udpcaster:alarm(app_config.cast_delay,tmr.ALARM_AUTO,
            function()
                ...
                if(network_latency > 5000000) then -- delay over 5 sec then wakeup broadcasting
                    master_socket:send(app_config.bc_port,broad_ip,sjson.encode({sk=1,did=0,cid=chipid,type="bc",aps=app_status}))
